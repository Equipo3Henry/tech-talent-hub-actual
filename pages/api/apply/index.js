    ...
    try {
        ...
        if(user.isPremium){
            ...
            if(user.limitFreeVacancies === 0){
                return res.status(403).json({
                    success: false,
                    message:"You've reached the maximum number of free applications this day. Please wait until tomorrow to apply again.",
                });
            }else{
                const result = await prisma.user.update({
                    where: { id: userId },
                    data: {
                        ...
                        limitFreeVacancies: user.limitFreeVacancies--,
                    },
                });
